...
const faqs = [
  ...
  { q: "Can I extend my rental period?", short: "Yes — just call us before it expires.", full: "Absolutely! Simply contact us before your current rental period ends and we'll extend it. Daily, weekly, and monthly extensions are available." },
  { q: "Do you have a mileage limit?", short: "250 km/day included; extra km available.", full: "Our standard rentals include 250 km per day. Weekly rentals include 1,500 km and monthly rentals include 4,500 km. Additional kilometers can be purchased at competitive rates." },
];
    ...
    <section id="faq" className="section-padding bg-background">
      <div className="container">
        <div className="text-center mb-16">
          ...
          <p className="text-muted-foreground mt-4 max-w-lg mx-auto">Everything you need to know about renting with Al Emad.</p>
        </div>
        ...
        <div className="max-w-3xl mx-auto grid gap-4">
          {faqs.map((faq, i) => (
            <div
              key={i}
              className="border border-border rounded-2xl overflow-hidden transition-all duration-300 cursor-pointer bg-background hover:border-primary/20"
              onClick={() => setOpenIdx(openIdx === i ? null : i)}
            >
              <div className="p-6">
                <div className="flex items-start gap-4">
                  <ChevronRight
                    className={`w-5 h-5 text-primary shrink-0 mt-0.5 transition-transform duration-300 ${openIdx === i ? "rotate-90" : ""}`}
                  />
                  <div className="flex-1">
                    <h3 className="font-semibold text-foreground font-sans text-base">{faq.q}</h3>
                    <p className="text-sm mt-1 text-muted-foreground/70">{faq.short}</p>
                    <div
                      className="overflow-hidden transition-all duration-500"
                      style={{
                        maxHeight: openIdx === i ? "200px" : "0px",
                        opacity: openIdx === i ? 1 : 0,
                        marginTop: openIdx === i ? "12px" : "0px",
                      }}
                    >
