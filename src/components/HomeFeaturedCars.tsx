...
const featuredIds = ["kia-pegas-2025", "mazda-3-2025", "hyundai-creta-2022", "kia-k5-2023", "peugeot-3008-2025", "infiniti-q50-2023"];

const HomeFeaturedCars = () => {
  const { data: allCars = [], isLoading } = useFleetCars();
  const featured = allCars.filter((c) => featuredIds.includes(c.id)).slice(0, 6);

  // If no featured found from IDs, show first 6
  const displayCars = featured.length > 0 ? featured : allCars.slice(0, 6);
