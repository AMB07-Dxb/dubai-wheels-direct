...
const iconMap: Record<string, any> = { gift: Gift, percent: Percent, tag: Tag };

const PromosSection = () => {
  const { data: config } = useSiteConfig();
  const siteConfig = { ...fallbackConfig, ...(config as any) };
  const promos = siteConfig.promos || fallbackConfig.promos;
  const waLink = siteConfig.whatsapp?.link || fallbackConfig.whatsapp.link;
  const bannerBg = siteConfig.banners?.promoBannerBg || fallbackConfig.banners?.promoBannerBg;
